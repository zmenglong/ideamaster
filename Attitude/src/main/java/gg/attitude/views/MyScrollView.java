...
public class MyScrollView extends ScrollView {
    ...
    private static final int ANIM_TIME = 300; // 松开手指�?, 界面回到正常位置�?要的动画时间

    private float startY;// 手指按下时的Y�?, 用于在移动时计算移动距离,如果按下时不能上拉和下拉�?
    // 会在手指移动时更新为当前手指的Y�?

    // ui
    private View contentView; // ScrollView的唯�?内容控件
    private final Rect originalRect = new Rect();// 用于记录正常的布�?位置

    // flag
    private boolean canPullDown = false; // 是否可以继续下拉
    private boolean canPullUp = false; // 是否可以继续上拉
    private boolean isMoved = false; // 记录是否移动了布�?
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (contentView == null)
            return;
        // ScrollView中的唯一子控件的位置信息, 这个位置信息在整个控件的生命周期中保持不�?
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (isTouchOutOfScrollView) { // 如果移动到了当前ScrollView控件之外
            if (isMoved) {// 如果当前contentView已经被移�?, 首先把布�?移到原位�?
                boundBack();
            }
            return true;
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                // 在移动的过程中， 既没有滚动到可以上拉的程度， 也没有滚动到可以下拉的程�?
    ...
    /**
     * 将内容布�?移动到原位置 可以在UP事件中调�?, 也可以在其他�?要的地方调用, 如手指移动到当前ScrollView外时
     */
    private void boundBack() {
        if (!isMoved) {
            return; // 如果没有移动布局�? 则跳过执�?
        }
        // �?启动�?
